...
sector(room(_,Sector,_), Sector).
checksum(room(_,_,Checksum), Checksum).
...
run2(Sector) :-
    room_list(List),
    find_north_pole_sector(List, Sector).
...
room_list(List) :-
    ...
    room_list(List, Chars, _).

room_list([Room|List]) --> room_(Room), ['\n'], room_list(List).
room_list([Room]) --> room_(Room).

room_(room(Name, Sector, Checksum)) -->
    letters(Name),
    dash,
    dcg_basics:integer(Sector),
    ['['],
    letters(Checksum),
    [']'].
letters(T) --> dash, !, letters(T).
